...
public class BatAttackNode : ActionNode
{
    // 판정 설정
    ...
    private HashSet<IDamageable> mHitTargets = new HashSet<IDamageable>();
    ...
    private bool mIsAgentStopped;

    public BatAttackNode(EnemyBase owner, float hitTiming, float offset, float radius) : base(owner)
    {
    ...
    public override ENodeState Evaluate()
    {
        ...
        if ( isAttackState && !bHitProcessed && stateInfo.normalizedTime >= mHitTiming && stateInfo.normalizedTime < 1f)
        {
            // 1. 판정 직전에 남은 후딜레이 시간 계산
            // (전체 길이 * 남은 비율(1 - 현재진행도)) / 재생 속도
            float normalTime = Mathf.Min(stateInfo.normalizedTime, 1f);
            float remainingTime = (stateInfo.length * (1f - stateInfo.normalizedTime)) / Mathf.Max(stateInfo.speed, 0.01f);
    ...
    private void PerformHitCheck()
    {
        ...
        {
            if (mHitResults[i] != null && mHitResults[i].TryGetComponent<IDamageable>(out var target))
            {
